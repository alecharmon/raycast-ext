...
import $hgUW1$nodecrypto, {randomUUID as $hgUW1$randomUUID} from "node:crypto";
...
import {rename as $hgUW1$rename, rm as $hgUW1$rm, stat as $hgUW1$stat, mkdir as $hgUW1$mkdir, copyFile as $hgUW1$copyFile, writeFile as $hgUW1$writeFile} from "node:fs/promises";
...
function $cefc05764ce5eacd$export$dd6b79aaabe7bc37(fn, args, options) {
    ...
    const callback = (0, $hgUW1$useCallback)((...args)=>{
        ...
        async function handleError(error) {
            ...
            if (normalizedError.name === "AbortError") return normalizedError;
            if (callId === lastCallId.current) {
                // handle errors
                if (latestOnError.current) try {
                    await latestOnError.current(normalizedError);
                } catch (onErrorError) {
                    console.error("The onError callback failed", onErrorError);
                }
                else {
                    if ((0, $hgUW1$environment).launchType !== (0, $hgUW1$LaunchType).Background) try {
                        await (0, $c718fd03aba6111c$export$80e5033e369189f3)(normalizedError, {
                ...
                if (callId === lastCallId.current) set({
        ...
        let promiseOrPaginatedPromise;
        try {
            latestOnWillExecute.current?.(args);
            promiseOrPaginatedPromise = $cefc05764ce5eacd$var$bindPromiseIfNeeded(fnRef.current)(...args);
        } catch (error) {
            ...
            ({ data: data, hasMore: hasMore, cursor: cursor })=>{
                if (callId === lastCallId.current) {
                    ...
                    try {
                        ...
                        Promise.resolve(onDataResult).catch((error)=>console.error("The onData callback failed", error));
                    } catch (error) {
...
function $93381684554307cb$export$fe386877a696ae78(writeTo, context = []) {
    ...
    return {
        ...
        _abortsignal: function(signal) {
            write("abortsignal:");
            this.dispatch([
                signal.aborted,
                signal.aborted ? String(signal.reason) : undefined
            ]);
        },
        _formdata: function(formData) {
            write("formdata:");
...
const $c40d7eded38ca69c$var$noOptimisticValue = /* #__PURE__ */ Symbol("no optimistic value");
function $c40d7eded38ca69c$var$serializeCachedValue(value) {
    if (typeof value === "undefined") return "undefined";
    const serializedValue = JSON.stringify(value, (0, $e2e1ea6dd3b7d2e1$export$b644b65666fe0c18));
    if (typeof serializedValue === "undefined") throw new Error("Cached state values must be JSON serializable");
    return serializedValue;
}
function $c40d7eded38ca69c$var$deserializeCachedValue(serializedValue, fallback) {
    if (typeof serializedValue === "undefined") return fallback;
    if (serializedValue === "undefined") return undefined;
    try {
        return JSON.parse(serializedValue, (0, $e2e1ea6dd3b7d2e1$export$63698c10df99509c));
    } catch (error) {
...
function $c40d7eded38ca69c$export$14afb9e4c16377d3(key, initialState, config) {
    ...
    const [optimisticState, setOptimisticState] = (0, $hgUW1$useState)($c40d7eded38ca69c$var$noOptimisticValue);
    const pendingWriteRef = (0, $hgUW1$useRef)(null);
    const timeoutRef = (0, $hgUW1$useRef)(null);
    const flushPendingWrite = (0, $hgUW1$useCallback)((resetOptimisticState = true)=>{
        ...
        if (pendingWrite) pendingWrite.cache.set(pendingWrite.key, pendingWrite.serializedValue);
        if (resetOptimisticState) setOptimisticState($c40d7eded38ca69c$var$noOptimisticValue);
    }, []);
    (0, $hgUW1$useEffect)(()=>{
        const pendingWrite = pendingWriteRef.current;
        if (pendingWrite && (pendingWrite.cache !== cache || pendingWrite.key !== key || pendingWrite.debounce !== config?.cacheWriteDebounce)) flushPendingWrite();
        setOptimisticState((state)=>state !== $c40d7eded38ca69c$var$noOptimisticValue && state.key !== key ? $c40d7eded38ca69c$var$noOptimisticValue : state);
    }, [
        cache,
        config?.cacheWriteDebounce,
        flushPendingWrite,
        key
    ]);
    (0, $hgUW1$useEffect)(()=>{
    ...
    const state = (0, $hgUW1$useMemo)(()=>{
        if (optimisticState !== $c40d7eded38ca69c$var$noOptimisticValue && optimisticState.key === key) return optimisticState.value;
        return $c40d7eded38ca69c$var$deserializeCachedValue(cachedState, initialValueRef.current);
    }, [
        cachedState,
        initialValueRef,
        key,
    ...
    const setStateAndCache = (0, $hgUW1$useCallback)((updater)=>{
        ...
        const pendingWrite = pendingWriteRef.current;
        let currentValue;
        if (pendingWrite && pendingWrite.cache === cache && pendingWrite.key === currentKey) currentValue = pendingWrite.value;
        else try {
            currentValue = $c40d7eded38ca69c$var$deserializeCachedValue(cache.get(currentKey), initialState);
        } catch (error) {
            console.error("Could not get Cache data:", error);
        ...
        const serializedValue = $c40d7eded38ca69c$var$serializeCachedValue(newValue);
        if (pendingWrite && (pendingWrite.cache !== cache || pendingWrite.key !== currentKey)) flushPendingWrite(false);
        ...
        if (typeof debounce === "undefined" || debounce === null || debounce < 0) {
            ...
            setOptimisticState($c40d7eded38ca69c$var$noOptimisticValue);
        } else {
            const newPendingWrite = {
                cache: cache,
                key: currentKey,
            ...
            timeoutRef.current = setTimeout(()=>{
                if (pendingWriteRef.current !== newPendingWrite) return;
                pendingWriteRef.current = null;
                timeoutRef.current = null;
                cache.set(newPendingWrite.key, newPendingWrite.serializedValue);
                setOptimisticState((state)=>state !== $c40d7eded38ca69c$var$noOptimisticValue && state.key === newPendingWrite.key ? $c40d7eded38ca69c$var$noOptimisticValue : state);
            }, debounce);
        }
        return newValue;
    }, [
...
function $a7f3824c7be647eb$export$b15740c74e256244(fn, args, options) {
   ...
   */ const { initialData: initialData, keepPreviousData: keepPreviousData, internal_cacheKeySuffix: internal_cacheKeySuffix, cacheWriteDebounce: cacheWriteDebounce, ...usePromiseOptions } = options || {};
    const lastUpdateFrom = (0, $hgUW1$useRef)(null);
    const cacheKey = (0, $e2e1ea6dd3b7d2e1$export$d6af199866bfb566)(args || []) + internal_cacheKeySuffix;
    const [cachedData, mutateCache] = (0, $c40d7eded38ca69c$export$14afb9e4c16377d3)(cacheKey, $a7f3824c7be647eb$var$emptyCache, {
        cacheNamespace: (0, $e2e1ea6dd3b7d2e1$export$d6af199866bfb566)(fn),
        cacheWriteDebounce: cacheWriteDebounce
    });
    ...
    const currentPaginationArgs = paginationArgsRef.current?.cacheKey === cacheKey ? paginationArgsRef.current.pagination : undefined;
    ...
    if (currentPaginationArgs && currentPaginationArgs.page > 0 && state.data) returnedData = state.data;
    else if (currentLastUpdateFrom === "promise") returnedData = laggyDataRef.current;
    ...
    const latestCacheKey = (0, $bfcf6ee368b3bd9f$export$d4b699e2c1148419)(cacheKey);
...
async function $07de1abb43355e7b$var$getRequestInfoCacheKey(requestInfo) {
    if (typeof requestInfo === "string" || requestInfo instanceof URL) return requestInfo.toString();
    const body = requestInfo.body === null ? undefined : new Uint8Array(await requestInfo.clone().arrayBuffer());
    return (0, $e2e1ea6dd3b7d2e1$export$d6af199866bfb566)({
        body: body,
...
function $07de1abb43355e7b$var$getUrlFactoryErrorCacheKey(url, error) {
    ...
    return `url-factory-error:${(0, $e2e1ea6dd3b7d2e1$export$d6af199866bfb566)([
        url.toString(),
        normalizedError
    ])}`;
}
function $07de1abb43355e7b$var$combineAbortSignals(...signals) {
    const definedSignals = signals.filter((signal)=>Boolean(signal));
...
function $07de1abb43355e7b$export$d852f5f778460fa4(url, options) {
    const { parseResponse: parseResponse, mapResult: mapResult, initialData: initialData, cacheWriteDebounce: cacheWriteDebounce, execute: execute, keepPreviousData: keepPreviousData, onError: onError, onData: onData, onWillExecute: onWillExecute, failureToastOptions: failureToastOptions, ...fetchOptions } = options || {};
    ...
    const paginatedUrlRef = (0, $hgUW1$useRef)(null);
    const firstPageErrorRef = (0, $hgUW1$useRef)(null);
    const [paginatedRequest, setPaginatedRequest] = (0, $hgUW1$useState)(null);
    const [resolvedRequest, setResolvedRequest] = (0, $hgUW1$useState)(null);
    ...
    (0, $hgUW1$useEffect)(()=>{
        ...
        (async ()=>{
            ...
            try {
                const requestInfo = paginatedUrl({
                    page: 0
                });
                cacheKey = await $07de1abb43355e7b$var$getRequestInfoCacheKey(requestInfo);
            } catch (error) {
                firstPageError = {
                    error: error
                };
                cacheKey = $07de1abb43355e7b$var$getUrlFactoryErrorCacheKey(paginatedUrl, error);
            }
            if (cancelled) return;
            firstPageErrorRef.current = firstPageError;
            setPaginatedRequest((previous)=>{
                if (previous?.cacheKey === cacheKey) return previous;
                return {
                    cacheKey: cacheKey,
                    requestInfo (pagination) {
                        const currentUrl = paginatedUrlRef.current;
                        if (!currentUrl) throw new Error("The paginated URL factory is not ready");
                        if (pagination.page === 0) {
                            const firstPageError = firstPageErrorRef.current;
                            if (firstPageError) throw firstPageError.error;
                        }
    ...
    (0, $hgUW1$useEffect)(()=>{
        ...
        let cancelled = false;
        $07de1abb43355e7b$var$getRequestInfoCacheKey(url).then((cacheKey)=>{
            if (!cancelled) setResolvedRequest({
                request: url,
                cacheKey: cacheKey
            });
        }, (error)=>{
            if (!cancelled) setResolvedRequest({
                request: url,
                cacheKey: `request-error:${(0, $e2e1ea6dd3b7d2e1$export$d6af199866bfb566)(String(error))}`
            });
        });
        return ()=>{
            cancelled = true;
    ...
    const paginatedFn = (0, $hgUW1$useCallback)((url, options)=>async (pagination)=>{
            ...
            const signal = $07de1abb43355e7b$var$combineAbortSignals(abortable.current?.signal, options?.signal, requestInfo instanceof Request ? requestInfo.signal : undefined);
    ...
    const fn = (0, $hgUW1$useCallback)(async (url, options)=>{
        ...
        const signal = $07de1abb43355e7b$var$combineAbortSignals(abortable.current?.signal, options?.signal, url instanceof Request ? url.signal : undefined);
        const res = await fetch(request, {
            ...options,
            signal: signal
        });
    ...
    const promise = (0, $hgUW1$useMemo)(()=>{
        if (isPaginated) return paginatedFn;
        return fn;
    }, [
        isPaginated,
        fn,
    ...
    const requestCacheKey = !isPaginated && url instanceof Request && resolvedRequest?.request === url ? resolvedRequest.cacheKey : null;
    ...
    const result = (0, $a7f3824c7be647eb$export$b15740c74e256244)(promise, [
        requestInfo,
        fetchOptions
    ], {
        ...useCachedPromiseOptions,
        execute: isPaginated ? execute !== false && paginatedRequest !== null : !isResolvingRequestCacheKey && execute,
        internal_cacheKeySuffix: (isPaginated ? paginatedRequest?.cacheKey : requestCacheKey || "") + (0, $e2e1ea6dd3b7d2e1$export$d6af199866bfb566)(mapResultRef.current) + (0, $e2e1ea6dd3b7d2e1$export$d6af199866bfb566)(parseResponseRef.current),
        abortable: abortable
    });
    if (isPaginated && execute !== false && paginatedRequest === null || isResolvingRequestCacheKey) return {
        ...result,
        isLoading: true
    };
    return result;
...
function $915f026f98ccae1a$export$3f4d948c82873887(command, optionsOrArgs, options) {
    const { parseOutput: parseOutput, input: input, onData: onData, onWillExecute: onWillExecute, initialData: initialData, cacheWriteDebounce: cacheWriteDebounce, execute: execute, keepPreviousData: keepPreviousData, onError: onError, failureToastOptions: failureToastOptions, ...execOptions } = Array.isArray(optionsOrArgs) ? options || {} : optionsOrArgs || {};
    const useCachedPromiseOptions = {
        initialData: initialData,
        cacheWriteDebounce: cacheWriteDebounce,
        execute: execute,
...
/* eslint-disable @typescript-eslint/ban-ts-comment */ /* eslint-disable @typescript-eslint/no-explicit-any */ /* eslint-disable no-control-regex */ /* eslint-disable no-useless-escape */ /* eslint-disable @typescript-eslint/no-unused-expressions */ /* eslint-disable @typescript-eslint/no-unused-vars */ 
...
async function $ba5a1846053848df$var$cache(url, destination, fetchOptions) {
    const temporaryDestination = `${destination}.${(0, $hgUW1$randomUUID)()}.tmp`;
    try {
        if (typeof url === "object" || url.startsWith("http://") || url.startsWith("https://")) await $ba5a1846053848df$var$cacheURL(url, temporaryDestination, fetchOptions);
        else if (url.startsWith("file://")) await $ba5a1846053848df$var$cacheFile((0, $hgUW1$normalize)(decodeURIComponent(new URL(url).pathname)), temporaryDestination, fetchOptions?.signal ? fetchOptions.signal : undefined);
        else throw new Error("Only HTTP(S) or file URLs are supported");
        await (0, $hgUW1$rename)(temporaryDestination, destination);
    } catch (error) {
        await (0, $hgUW1$rm)(temporaryDestination, {
            force: true
        }).catch(()=>undefined);
        throw error;
    }
...
async function* $ba5a1846053848df$var$streamJsonFile(filePath, pageSize, abortSignal, dataPath, filterFn, transformFn) {
    ...
    try {
        for await (const data of pipeline){
            ...
            const items = transformFn && Array.isArray(transformedData) ? transformedData : [
                transformedData
            ];
            for (const item of items){
                if (!filterFn || filterFn(item)) page.push(item);
...
function $ba5a1846053848df$export$48c74caed1925dc8(url, options) {
    const { initialData: initialData, cacheWriteDebounce: cacheWriteDebounce, execute: execute, keepPreviousData: keepPreviousData, onError: onError, onData: onData, onWillExecute: onWillExecute, failureToastOptions: failureToastOptions, dataPath: dataPath, filter: filter, transform: transform, pageSize: pageSize = 20, ...fetchOptions } = options ?? {};
...
async function $da578bce0e88fb2a$export$4cff4f142f384da7(databasePath, query, options) {
    ...
    try {
        return $da578bce0e88fb2a$var$executeWithNodeSQLite(sqlite3, databasePath, query, abortSignal);
    } catch (error) {
        if ($da578bce0e88fb2a$var$isDatabaseBusy(error)) {
            ...
            const workaroundCopiedDb = await $da578bce0e88fb2a$var$createDatabaseCopy(databasePath, abortSignal);
            return $da578bce0e88fb2a$var$executeWithNodeSQLite(sqlite3, workaroundCopiedDb, query, abortSignal);
        }
        throw error;
    }
}
function $da578bce0e88fb2a$var$executeWithNodeSQLite(sqlite3, databasePath, query, abortSignal) {
    const db = new sqlite3.DatabaseSync(databasePath, {
        open: false,
        readOnly: true
    });
    let opened = false;
...
function $da578bce0e88fb2a$var$isDatabaseBusy(error) {
    if (!(error instanceof Error)) return false;
    const errorWithCode = error;
    return errorWithCode.errcode === 5 || errorWithCode.errcode === 14 || error.message.includes("(5)") || error.message.includes("(14)");
}
async function $da578bce0e88fb2a$var$createDatabaseCopy(databasePath, abortSignal) {
    ...
    $da578bce0e88fb2a$var$checkAborted(abortSignal);
    const workaroundCopiedDb = (0, $hgUW1$nodepath).join(tempFolder, "db.db");
    try {
        await (0, $hgUW1$copyFile)(databasePath, workaroundCopiedDb);
    } catch (error) {
        if (process.platform === "darwin" && error instanceof Error && "code" in error && error.code === "EPERM") throw new $da578bce0e88fb2a$export$cb7dc23b705f5c86("You do not have permission to access the database.");
        throw error;
    }
...
async function $da578bce0e88fb2a$var$sqliteFallback(databasePath, query, options) {
    ...
    if (error || exitCode !== 0 || signal !== null) {
        if (process.platform === "darwin" && stderrResult.includes("authorization denied")) throw new $da578bce0e88fb2a$export$cb7dc23b705f5c86("You do not have permission to access the database.");
...
function $e88ea8aee5e4e2a5$export$d74ef0af94fb1db6(databasePath, query, options) {
    ...
    const abortable = (0, $hgUW1$useRef)(null);
    const handleError = (0, $hgUW1$useCallback)((error)=>{
        console.error(error);
        if ((0, $da578bce0e88fb2a$export$e11674744930cd15)(error)) setPermissionView(/*#__PURE__*/ (0, $hgUW1$jsx)($e88ea8aee5e4e2a5$var$PermissionErrorScreen, {
            ...
            if (latestOptions.current.onError) return latestOptions.current.onError(error);
    ...
    const handleData = (0, $hgUW1$useCallback)((data)=>{
        setPermissionView(null);
        return latestOptions.current.onData?.(data);
    }, [
        latestOptions
    ]);
    const fn = (0, $hgUW1$useCallback)(async (databasePath, query)=>{
...
function $79498421851e7e84$export$87c0cf8eb5a167e0(props) {
    ...
    const reset = (0, $hgUW1$useCallback)((values)=>{
        ...
        Object.entries(refs.current).forEach(([id, ref])=>{
            if (!values || !Object.prototype.hasOwnProperty.call(values, id)) ref?.reset();
        });
...
function $571cbdb5f3bef8a7$export$835fc3bd312a97c(prompt, options = {}) {
    ...
    const abortable = (0, $hgUW1$useRef)(null);
    const requestIdRef = (0, $hgUW1$useRef)(0);
    const { isLoading: isLoading, error: error, revalidate: revalidate } = (0, $cefc05764ce5eacd$export$dd6b79aaabe7bc37)(async (prompt, creativity, shouldStream, model)=>{
        ...
        if (shouldStream === false) {
            ...
            stream.on("data", (data)=>{
                if (requestId === requestIdRef.current) setData((x)=>x + data);
            });
            await stream;
        }
    }, [
...
function $ae6377cf02b7ee4b$var$getNewFrecency(item) {
    const now = Date.now();
    const visitAgeInDays = item ? (now - item.lastVisited) / $ae6377cf02b7ee4b$var$MS_PER_DAY : 0;
    const decay = Math.exp(-(Math.log(2) / $ae6377cf02b7ee4b$var$HALF_LIFE_DAYS) * visitAgeInDays);
    const totalVisitValue = (item?.frecency ?? 0) * decay + $ae6377cf02b7ee4b$var$VISIT_TYPE_POINTS.Default;
    return {
        lastVisited: now,
...
function $a613cd7627dc664f$var$normalizeTokenResponse(tokens) {
...
class $a613cd7627dc664f$export$905bf243392675e9 {
   ...
   */ async authorize() {
    ...
    async fetchTokens({ authRequest: authRequest, authorizationCode: authorizationCode }) {
        ...
        return $a613cd7627dc664f$var$normalizeTokenResponse(tokens);
    }
    async refreshTokens({ token: token }) {
        ...
        if (!response.ok) {
            ...
            return $a613cd7627dc664f$var$normalizeTokenResponse(tokenResponse);
        }
    }
}
...
let $610e71f8af383779$var$accessToken;
let $610e71f8af383779$var$accessTokenPromise = null;
let $610e71f8af383779$var$idTokenPromise = null;
let $610e71f8af383779$var$onAuthorizePromise = null;
function $610e71f8af383779$export$d6bc6f4110e73ed6(options) {
    ...
    const authorize = ()=>{
        $610e71f8af383779$var$accessTokenPromise ??= (personalAccessToken ? Promise.resolve(personalAccessToken) : options.authorize().then((token)=>({
                token: token,
                type: "oauth"
            }))).catch((error)=>{
            $610e71f8af383779$var$accessTokenPromise = null;
            throw error;
        });
        return $610e71f8af383779$var$accessTokenPromise;
    };
    const getIdToken = ()=>{
        if (!options.client) return undefined;
        $610e71f8af383779$var$idTokenPromise ??= options.client.getTokens().then((tokens)=>tokens?.idToken, (error)=>{
            $610e71f8af383779$var$idTokenPromise = null;
            throw error;
        });
        return (0, $hgUW1$react).use($610e71f8af383779$var$idTokenPromise);
    };
    const runOnAuthorize = (accessToken, idToken)=>{
        if (!options.onAuthorize) return;
        $610e71f8af383779$var$onAuthorizePromise ??= Promise.resolve(options.onAuthorize({
            ...accessToken,
            idToken: idToken
        })).catch((error)=>{
            $610e71f8af383779$var$onAuthorizePromise = null;
            throw error;
        });
        return $610e71f8af383779$var$onAuthorizePromise;
    };
    if ((0, $hgUW1$environment).commandMode === "no-view") return (fn)=>{
        const noViewFn = async (props)=>{
            $610e71f8af383779$var$accessToken = await authorize();
            const idToken = (await options.client?.getTokens())?.idToken;
            await runOnAuthorize($610e71f8af383779$var$accessToken, idToken);
            return fn(props);
        };
        return noViewFn;
    };
    return (Component)=>{
        const WrappedComponent = (props)=>{
            $610e71f8af383779$var$accessToken = personalAccessToken || (0, $hgUW1$react).use(authorize());
            const idToken = getIdToken();
            const onAuthorize = runOnAuthorize($610e71f8af383779$var$accessToken, idToken);
            if (onAuthorize) (0, $hgUW1$react).use(onAuthorize);
            return /*#__PURE__*/ (0, $hgUW1$react).createElement(Component, props);
        };
...
function $610e71f8af383779$export$84773cc9bf7dbe87() {
    if (!$610e71f8af383779$var$accessToken) throw new Error("getAccessToken must be used when authenticated (eg. used inside `withAccessToken`)");
    return $610e71f8af383779$var$accessToken;
}
...
function $7b2399a584bd1625$var$getProtocol() {
    if (process.env.RAYCAST_SCHEME) return `${process.env.RAYCAST_SCHEME}://`;
    return "raycast://";
}
...
async function $179449bbef1d262b$export$8982d27dc997624f(script, optionsOrArgs, options) {
    ...
    const { humanReadableOutput: humanReadableOutput, language: language, parseOutput: parseOutput, timeout: timeout, ...execOptions } = Array.isArray(optionsOrArgs) ? options || {} : optionsOrArgs || {};
    ...
    return (parseOutput ?? (0, $c86e3701e284ece4$export$a8f5efe603803b77))({
        stdout: stdout,
        stderr: stderr,
        error: error,
        exitCode: exitCode,
...
async function $0c3b8057691eb3cf$export$fc36b0024b60e761(script, options) {
    ...
    const { parseOutput: parseOutput, timeout: timeout, ...execOptions } = options || {};
